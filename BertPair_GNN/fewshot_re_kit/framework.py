...
class FewShotREFramework:
    ...
    def train(self,
        ...
        wfile = open(model_name.split('/')[0], 'a')
        # Init
        if bert_optim:
            ...
            optimizer = pytorch_optim(parameters_to_optimize_encoder, learning_rate, weight_decay=weight_decay)
            scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=lr_step_size)
            if sup_cls:
                optimizer_sup = pytorch_optim(parameters_to_optimize_classifier, learning_rate, weight_decay=weight_decay)
        ...
        for it in range(start_iter, start_iter + train_iter):
            if pair:
                batch, label, support_batch , _, _, _= next(self.train_data_loader)
                if torch.cuda.is_available():
                    for k in batch:
                        batch[k] = batch[k].cuda()
                    label = label.cuda()
                    for k in support_batch:
                        support_batch[k] = support_batch[k].cuda()
                logits, pred, sup_logits, sup_labels = model(batch, N_for_train, K, Q * N_for_train + na_rate * Q, support_batch, training=True)
                loss = model.loss(logits, label)/ float(grad_iter)
                if sup_cls:
                    loss += model.loss(sup_logits, sup_labels)/ float(grad_iter)
            else:
                support, query, label, _, _, _ = next(self.train_data_loader)
                if torch.cuda.is_available():
                    for k in support:
                ...
                logits, pred, sup_cost  = model(support, query,
                        N_for_train, K, Q, training=True) #* N_for_train + na_rate * Q
                loss = (model.loss(logits, label)+sup_cost) / float(grad_iter)
            ...
            if (it+1) % 1 == 0 and sup_cls and not pair:
                optimizer_sup.step()
                scheduler_sup.step()
            ...
            if (it + 1) % val_step == 0:
                ...
                model.train()
                wfile.write(str(N_for_eval)+'\t'+str(K)+'\t'+str(acc)+'\n')
    ...
    def eval(self,
        ...
        with torch.no_grad():
            for it in range(eval_iter):
                if pair:
                    batch, label, support_batch, label_reference,\
                        support_origin, query_origin = next(eval_dataset)
                    support_origin = support_origin[0]
                    query_origin = query_origin[0]
                    label_reference = label_reference[0]
                    if torch.cuda.is_available():
                        for k in batch:
                            batch[k] = batch[k].cuda()
                        label = label.cuda()
                        for k in support_batch:
                            support_batch[k] = support_batch[k].cuda()
                    logits, pred, _, _ = model(batch, N, K, Q * N + Q * na_rate,support_batch,training=False)
                    for i in range(len(query_origin)):
                        query_origin[i]['predict'] = label_reference[int(pred[i].cpu())]
                    task = {}
                    task['support'] = support_origin
                    task['query'] = query_origin
                    tasks.append(task)
                else:
                    support, query, label, _, _, _ = next(eval_dataset)
                    if torch.cuda.is_available():
                        for k in support:
                            support[k] = support[k].cuda()
                        for k in query:
                            query[k] = query[k].cuda()
                        label = label.cuda()
                    logits, pred,_ = model(support, query, N, K, Q,training=False) #* N + Q * na_rate
                right = model.accuracy(pred, label)
                iter_right += self.item(right.data)
                iter_sample += 1
