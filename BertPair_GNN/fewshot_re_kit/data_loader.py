...
class FewRelDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return support_set, query_set, query_label #, None, None, None
...
def collate_fn(data):
    ...
    return batch_support, batch_query, batch_label, None, None, None
...
class FewRelDatasetPair(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        support_fusion_set = {'word': [], 'mask': [], 'seg': []}
        query_label = []
        Q_na = int(self.na_rate * self.Q)
        na_classes = list(filter(lambda x: x not in target_classes,  
            self.classes))
        ...
        for i, class_name in enumerate(target_classes):
            ...
            for j in indices:
                word  = self.__getraw__(
                        self.json_data[class_name][j])
                this_json = self.json_data[class_name][j]
                this_json['label'] = class_name
                if count < self.K:
                    support.append(word)
                    support_origin.append(this_json)
                else:
                    query.append(word)
                    query_origin.append(this_json)
                count += 1
        ...
        for word_s1 in support:
            for word_s2 in support:
                if word_s1 >= word_s2:
                    continue
                SEP = self.encoder.tokenizer.convert_tokens_to_ids(['[SEP]'])
                CLS = self.encoder.tokenizer.convert_tokens_to_ids(['[CLS]'])
                new_word = CLS + word_s1 + SEP + word_s2 + SEP
                word_tensor = torch.zeros((self.max_length)).long()
                for i in range(min(self.max_length, len(new_word))):
                    word_tensor[i] = new_word[i]
                mask_tensor = torch.zeros((self.max_length)).long()
                ...
                seg_tensor = torch.ones((self.max_length)).long()
                seg_tensor[:min(self.max_length, len(word_support) + 1)] = 0
                support_fusion_set['word'].append(word_tensor)
                support_fusion_set['mask'].append(mask_tensor)
                support_fusion_set['seg'].append(seg_tensor)

        return fusion_set, query_label, support_fusion_set, labele_reference, support_origin, query_origin
...
def collate_fn_pair(data):
    ...
    support_batch_set = {'word': [], 'seg': [], 'mask': []}
    batch_label = []
    fusion_sets, query_labels, support_fusion_sets, label_reference,\
        support_json, query_json= zip(*data)
    ...
    batch_label = torch.tensor(batch_label)
    for i in range(len(support_fusion_sets)):
        for k in support_fusion_sets[i]:
            support_batch_set[k] += support_fusion_sets[i][k]
    for k in support_batch_set:
        support_batch_set[k] = torch.stack(support_batch_set[k], 0)
    return batch_set, batch_label, support_batch_set, label_reference, support_json, query_json
