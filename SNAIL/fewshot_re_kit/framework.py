...
class FewShotREFramework:
    ...
    def train(self,
        ...
        wfile = open('krresults/'+save_ckpt.split('/')[1],'a')
        # Init
        if bert_optim:
        ...
        #    optimizer = pytorch_optim(filter(lambda p:p.requires_grad, model.parameters()),#model.parameters(),
        #            learning_rate, weight_decay=weight_decay)
        #    if self.adv:
        #        optimizer_encoder = pytorch_optim(model.parameters(), lr=adv_enc_lr)
        #    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=lr_step_size)
            parameters_to_optimize_classifier_ = [v for k, v in model.named_parameters() if k.find("try_linear") >= 0]
            parameters_to_optimize_classifier = list(filter(lambda p:p.requires_grad, parameters_to_optimize_classifier_))
            parameters_to_optimize_encoder_ = [v for k, v in model.named_parameters() if k.find("try_linear") < 0]
            parameters_to_optimize_encoder = list(filter(lambda p:p.requires_grad, parameters_to_optimize_encoder_))
        ...
        for it in range(start_iter, start_iter + train_iter):
            if pair:
                ...
                if torch.cuda.is_available():
                    for k in support:
                        support[k] = torch.autograd.Variable(support[k].cuda())
                    for k in query:
                        query[k] = torch.autograd.Variable(query[k].cuda())
            ...
            # print(not updated)
            # time.sleep(5)
            if sup_cls and not pair and not updated:
              #  print(2)
                # time.sleep(10)
                scheduler_sup.step()
                optimizer_sup.zero_grad()
            ...
            if pair:
                ...
                if torch.cuda.is_available():
                    ...
                    for k in query:
                        query[k] = torch.autograd.Variable(query[k].cuda())
                    label = torch.autograd.Variable(label.cuda())
                logits, pred, _ = model(support, query, N, K, Q ) #* N + Q * na_rate
            right = model.accuracy(pred, label)
            iter_right += self.item(right.data)
            iter_sample += 1
