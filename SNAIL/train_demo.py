...
def main():
    ...
    parser.add_argument('--sup_cost', type=int, default=0, help='use sup classifier')
    ...
    if encoder_name == 'cnn':
        try:
            if opt.language == 'chn':
                ...
                glove_word2id = json.load(open('./pretrain/chinese_emb/word2id.json'))
            else:
    ...
    if model_name == 'proto':
        model = Proto(sentence_encoder, hidden_size=opt.hidden_size, use_sup_cost=sup_cost)
    elif model_name == 'gnn':
        model = GNN(sentence_encoder, N, use_sup_cost=sup_cost)
    elif model_name == 'snail':
        print("HINT: SNAIL works only in PyTorch 0.3.1")
        print(sup_cost)
        model = SNAIL(sentence_encoder, N, K, use_sup_cost=sup_cost)
    ...
    if not opt.only_test:
        if encoder_name == 'bert':
            ...
            bert_optim = False
        framework.train(model, prefix, batch_size, trainN, N, K, Q, grad_iter=int(opt.grad_iter),
                pytorch_optim=pytorch_optim, load_ckpt=opt.load_ckpt, save_ckpt=ckpt,
                na_rate=opt.na_rate, val_step=opt.val_step, fp16=opt.fp16, pair=opt.pair, 
                train_iter=opt.train_iter, val_iter=opt.val_iter, bert_optim=bert_optim, sup_cls=sup_cost,
